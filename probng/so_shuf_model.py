...
def acc(file, tokenizer_name, model_name):
    ...
    train = train.rename(columns={train.columns[0]: 'answer', train.columns[1]: 'tr_te',
                              ...
                              train.columns[7]: 'sent_6', train.columns[8]: 'sent_7'})
    train = train.fillna('.')
    ...
    texts1 = (train_tr['sent_1'].map(str) +
                    ...
                    + '.'+ train_tr['sent_5'].map(str)+ '.'+ train_tr['sent_6'].map(str)+ '.'+ train_tr['sent_7'].map(str))
    ...
    #decoder_input_ids = input_ids1,input_ids= input_ids1,
    with torch.no_grad():
        last_hidden_states1 = model(decoder_input_ids = input_ids1,input_ids= input_ids1, attention_mask=attention_mask1)
    ...
    #decoder_input_ids = input_ids2,input_ids= input_ids2,
    with torch.no_grad():
        last_hidden_states2 = model(decoder_input_ids = input_ids2,input_ids= input_ids2, attention_mask=attention_mask2)
...
files = ['/content/bg-so_shuf.csv','ca-so_shuf.csv','/content/en-so_shuf.csv',
         '/content/fr-so_shuf.csv','/content/hyw-so_shuf.csv',
         '/content/la-so_shuf.csv',
          '/content/ru-so_shuf.csv', '/content/cs-so_shuf.csv',
         '/content/tr-so_shuf.csv','/content/sr-so_shuf.csv']
