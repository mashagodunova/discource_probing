...
def acc(file, tokenizer_name, model_name):
    ...
    train = train.rename(columns={train.columns[0]: 'answer', train.columns[1]: 'tr_te',
                              ...
                              train.columns[4]: 'conn'})
    train = train.fillna('.')
    train['conn'] = train['conn'].apply(lambda x: x[:-1])
    train_tr = train[train['tr_te']=='tr']
    train_te = train[train['tr_te']=='te']
    tokenizer = tokenizer_name
    model = model_name
    labels1 = train_tr['conn']
    texts1 =(train_tr['sent_1'].map(str) +
                    '.'+ train_tr['sent_2'].map(str))
...
files = ['/content/bg-output.csv','ca-output.csv','/content/en-output.csv',
         '/content/fr-output.csv','/content/hyw-output.csv',
         '/content/la-output.csv',
          '/content/ru-output.csv', '/content/cs-output.csv',
         '/content/tr-output.csv','/content/sr-output.csv']
