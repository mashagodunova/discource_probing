...
for file_path in files:
    ...
    for index in df.index:
        if df['num_non'][index] < 7 and df['num_non'][index] > 11:
            df = df.drop([index])
    # df = df.drop(0, axis=1)
    # df = df.drop(0, axis=1)
    ...
    print('done')
    with open(f'{lang}-so_shuf.csv', 'w', encoding="utf-8") as newf:
        my_writer = csv.writer(newf, delimiter="\t", lineterminator="\n")
        for index, row in df_true.iterrows():
            my_writer.writerow([row[0], row[1], row[2], float('nan'),float('nan'),float('nan'),float('nan'), row['answer']])
        for index, row in df_true.iterrows():
            if row[3] is not None:
                my_writer.writerow([row[0], row[1], row[2], row[3], float('nan'),float('nan'),float('nan'), row['answer']])
        for index, row in df_true.iterrows():
            if row[4] is not None:
                my_writer.writerow([row[0],row[1], row[2], row[3], row[4], float('nan'), float('nan'),row['answer']])
        for index, row in df_true.iterrows():
            if row[5] is not None:
                my_writer.writerow([row[0], row[1], row[2], row[3], row[4], row[5], float('nan'), row['answer']])
        for index, row in df_true.iterrows():
            if row[6] is not None:
                my_writer.writerow([row[0], row[1], row[2], row[3], row[4], row[5], row[6], row['answer']])
        ...
        for index, row in df_false.iterrows():
            if row[6] is not None:
                my_writer.writerow([row[0], row[1], row[2], row[3], row[4], row[5], row[6], row['answer']])
    columns = ['sent_1', 'sent_2', 'sent_3', 'sent_4','sent_5','sent_6','sent_7','answer']
    new_df = pd.read_csv(f'{lang}-so_shuf.csv', delimiter="\t", lineterminator="\n", names = columns)
    ...
    with open(f'{lang}-so_shuf.csv', 'w', encoding="utf-8") as newf:
        ...
        for index, row in n_df.iterrows():
            my_writer.writerow([row['answer'], row['tr_te'], row['sent_1'], row['sent_2'],row['sent_3'], row['sent_4'],row['sent_5'], row['sent_6'], row['sent_7']])
