...
for file_path in files:
    ...
    with open(f'{lang}-sent_posit.csv', 'w', encoding="utf-8") as newf:
        ...
        for index, row in df_false.iterrows():
            ...
            fixed = df_false.loc[index, 4]
            df_false.loc[index, 4] = df_false.loc[index, change]
            df_false.loc[index, change] = fixed
            my_writer.writerow([row[0], row[1],row[2], row[3], row[4], row['answer']])
        for index, row in df_false.iterrows():
            foo1 = [1, 2, 3, 4, 5]
            if row[5] is not None:
                change1 = random.choice(foo1)
                fixed1 = df_false.loc[index, 4]
                df_false.loc[index, 4] = df_false.loc[index, change1]
                df_false.loc[index, change1] = fixed1
                my_writer.writerow([row[1],row[2], row[3],row[4], row[5], row['answer']])
        for index, row in df_false.iterrows():
            foo2 = [2, 3, 4, 5, 6]
            if row[6] is not None:
                change2 = random.choice(foo2)
                fixed2 = df_false.loc[index, 4]
                df_false.loc[index, 4] = df_false.loc[index, change2]
                df_false.loc[index, change2] = fixed2
                my_writer.writerow([row[2], row[3],row[4], row[5], row[6], row['answer']])
        for index, row in df_false.iterrows():
            foo2 = [ 3, 4, 5, 6, 7]
            if row[7] is not None:
                change2 = random.choice(foo2)
                fixed2 = df_false.loc[index, 5]
                df_false.loc[index, 5] = df_false.loc[index, change2]
                df_false.loc[index, change2] = fixed2
        ...
        #     if row[8] is not None:
        #         change2 = random.choice(foo2)
        #         fixed2 = df_false.loc[index, 6]
        #         df_false.loc[index, 6] = df_false.loc[index, change2]
        #         df_false.loc[index, change2] = fixed2
        #         my_writer.writerow([row[4], row[5], row[6], row[7], row[8], row['answer']])
    columns = ['sent_1', 'sent_2', 'sent_3', 'sent_4','sent_5','answer']
    new_df = pd.read_csv(f'{lang}-sent_posit.csv', delimiter="\t", lineterminator="\n", names=columns)
    ...
    with open(f'{lang}-sent_posit.csv', 'w', encoding="utf-8") as newf:
        ...
        for index, row in n_df.iterrows():
            my_writer.writerow([row['answer'], row['tr_te'], row['sent_1'], row['sent_2'],row['sent_3'], row['sent_4'],row['sent_5']])
